...
import { getGridItemClass } from "@/lib/utils/layout-utils"
      ...
      <div className="flex-1 overflow-auto">
        <div className="max-w-[1920px] mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 md:gap-4 p-3 md:p-4">
            {/* 시간대별 발전량 */}
            <div className={getGridItemClass(WIDGET_LAYOUT.hourlyGeneration)}>
              <HourlyGeneration refreshInterval={refreshInterval} />
            </div>

            {/* 일일 정보 */}
            <div className={getGridItemClass(WIDGET_LAYOUT.dailyInfo)}>
              <DailyInfo refreshInterval={refreshInterval} />
            </div>

            {/* 충전기 사용 정보 */}
            <div className={getGridItemClass(WIDGET_LAYOUT.chargerUsage)}>
              <ChargerUsage refreshInterval={refreshInterval} />
            </div>

            {/* 실시간 전기 거래 현황 */}
            <div className={getGridItemClass(WIDGET_LAYOUT.electricityTrading)}>
              <ElectricityTrading refreshInterval={refreshInterval} />
            </div>

            {/* 등록 충전소 정보 */}
            <div className={getGridItemClass(WIDGET_LAYOUT.stationInfo)}>
