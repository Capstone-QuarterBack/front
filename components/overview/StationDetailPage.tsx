...
export default function StationDetailPage() {
  ...
  const [chargerInfoMap, setChargerInfoMap] = useState<
    Map<number, ChargerInfoResponse>
  >(new Map());
  ...
  useEffect(() => {
    ...
    const loadChargerStatuses = async () => {
      try {
        ...
        console.log(
          `충전소 ${station.stationId}의 충전기 상태 정보 수신 완료:`,
          statuses
        );
      ...
      try {
        ...
        const promises = chargerStatuses.map(async (status) => {
          try {
            ...
            console.log(
              `충전기 ${status.evseId}의 상세 정보 요청 중... (상태: ${statusType})`
            );

            const info = await fetchChargerInfo(
              station.stationId,
              status.evseId,
              statusType
            );
