...
async function apiRequest<T>(
  ...
  if (useMockData) {
    ...
    return Promise.reject(
      new Error(`지원하지 않는 모의 데이터 엔드포인트: ${endpoint}`)
    );
  }
...
export async function fetchDischargeByHour(): Promise<DischargeByHourData[]> {
  try {
    ...
    const data = await apiRequest<DischargeByHourData[]>(
      "/dashboard/discharge-by-hour"
    );
...
export async function fetchChargerUsage(): Promise<ChargerUsageData[]> {
  try {
    ...
    const data = await apiRequest<ChargerUsageData[]>(
      "/dashboard/chargers/usage"
    );
...
export async function fetchStationOverview(): Promise<StationOverviewData[]> {
  try {
    ...
    data.forEach((station) => {
      // 한국 영토 내의 좌표인지 확인 (대략적인 범위)
      const isValidLatitude =
        station.latitude >= 33 && station.latitude <= 38.5;
      const isValidLongitude =
        station.longitude >= 124 && station.longitude <= 132;

      if (!isValidLatitude || !isValidLongitude) {
        console.warn(
          `유효하지 않은 좌표 (${station.stationName}): lat=${station.latitude}, lng=${station.longitude}`
        );

        // 유효하지 않은 좌표는 서울 시청 좌표로 대체 (실제 환경에서는 이렇게 하지 않는 것이 좋습니다)
        if (process.env.NEXT_PUBLIC_USE_MOCK_DATA === "true") {
          console.log(
            `좌표 보정: ${station.stationName}의 좌표를 서울 시청 좌표로 대체합니다.`
          );
          station.latitude = 37.5665 + (Math.random() - 0.5) * 0.02; // 약간의 랜덤성 추가
          station.longitude = 126.978 + (Math.random() - 0.5) * 0.02;
        }
      }
    });
